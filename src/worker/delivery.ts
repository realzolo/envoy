...
export async function processDelivery(job: Job<{ deliveryId: string }>) {
  ...
  try {
    ...
    await markDeterminateFailure(submission.attemptId, {
      ...
      message: error instanceof Error ? error.message : "Local message preparation failed"
    }, false);
    return
  }
  let result: ProviderSendResult;
  try {
    result = await account.module.sender.send({
    ...
    if (error instanceof CanonicalProviderError) {
      if (error.outcome !== "not_accepted") {
        await markUnknown(submission.attemptId, error.message);
        return
      }
      await markDeterminateFailure(submission.attemptId, {
    ...
    await markDeterminateFailure(submission.attemptId, {
      ...
      message: error instanceof Error ? error.message : "Provider rejected the request"
    }, false);
    return;
  }
  if (result.outcome === "unknown") {
    await markUnknown(submission.attemptId, result.reason);
    return
  }
  ...
  if (account.module.descriptor.type === "mock") {
    await applyCanonicalEvent(submission.providerAccountId, null, {
      id: crypto.randomUUID(),
      externalMessageId: result.externalMessageId,
      type: "accepted",
      occurredAt: new Date(),
      recipient: submission.recipientEmail,
      metadata: {}
    });
    await applyCanonicalEvent(submission.providerAccountId, null, {
      id: crypto.randomUUID(),
      externalMessageId: result.externalMessageId,
      type: submission.recipientEmail.includes("bounce") ? "bounced" : submission.recipientEmail.includes("complaint") ? "complained" : "delivered",
      occurredAt: new Date(Date.now() + 100),
      recipient: submission.recipientEmail,
      bounce: submission.recipientEmail.includes("bounce") ? {
        classification: "hard",
        message: "Simulated hard bounce"
      } : undefined,
      metadata: {}
    });
  }
