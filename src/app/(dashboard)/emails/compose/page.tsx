...
export default async function ComposeMessagePage() {
  ...
  return <div className="space-y-6">
    <div><Link href="/emails"
               className="mb-5 inline-flex items-center gap-1.5 text-xs text-zinc-600 hover:text-zinc-300">
      ...
      <div className="border-b border-zinc-800 pb-6"><h1 className="text-xl font-medium text-zinc-50">Send a test</h1>
        <p className="mt-1.5 max-w-2xl text-sm leading-6 text-zinc-500">Submit final rendered content through the same
          asynchronous delivery path used by service clients.</p>
      </div>
    </div>
    {data.services.length ? <MessageComposer services={data.services} senders={data.senders}/> :
      <section className="flex min-h-72 flex-col items-center justify-center border-y border-zinc-800 px-6 text-center">
        ...
        <p className="mt-2 max-w-md text-sm leading-6 text-zinc-500">Create a product and issue a service credential
          before submitting a test message.</p>
        <Link href="/api-keys"
              className="mt-5 inline-flex h-9 items-center gap-2 rounded-md bg-zinc-100 px-3 text-sm font-medium text-zinc-950">
