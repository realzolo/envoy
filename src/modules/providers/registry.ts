...
export function providerRegistry(type:ProviderType){const providerModule=modules.get(type);if(!providerModule)throw new Error(`Unsupported provider type: ${type}`);return providerModule}
export function providerDescriptors(){return [...modules.values()].map(providerModule=>providerModule.descriptor)}
