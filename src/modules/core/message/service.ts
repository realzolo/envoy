...
async function suppression(client: PoolClient, productId: string, email: string, listId?: string) {
  return Boolean((await client.query(`SELECT 1 FROM suppressions WHERE active=true AND email_normalized=$1 AND (expires_at IS NULL OR expires_at>now()) AND (scope_type='global' OR (scope_type='product' AND product_id=$2) OR (scope_type='list' AND product_id=$2 AND list_id=$3)) LIMIT 1`, [email.toLowerCase(), productId, listId ?? null])).rowCount)
}
...
export async function acceptMessage(args: {
  ...
  return transaction(async client => {
    const existing = await client.query<{
      ...
      payload_hash: string;
      accepted_at: Date
    }>("SELECT id,payload_hash,accepted_at FROM messages WHERE service_id=$1 AND idempotency_key=$2 FOR UPDATE", [args.identity.serviceId, args.idempotencyKey]);
    if (existing.rows[0]) {
    ...
    }>(`SELECT t.id AS template_id,tv.id AS version_id,tv.variables_schema AS schema,t.category,sp.id AS sender_profile_id
    ...
    if (!selected) throw new MessageValidationError(["The template, published version, or sender profile is unavailable."]);
    const issues = validateTemplateVariables(selected.schema, args.input.variables);
    if (issues.length) throw new MessageValidationError(issues);
    ...
    for (const recipient of args.input.to) {
      ...
      await client.query(`INSERT INTO deliveries(id,message_id,recipient_email,recipient_name,lifecycle_status,compliance_status) VALUES ($1,$2,$3,$4,$5,$6)`, [deliveryId, id, recipient.email.toLowerCase(), recipient.name ?? null, blocked ? "failed" : "queued", blocked ? "suppressed" : "clean"]);
      await client.query(`INSERT INTO outbox_events(id,aggregate_type,aggregate_id,event_type,payload) VALUES ($1,'delivery',$2,$3,$4)`, [createId("out"), deliveryId, blocked ? "delivery.suppressed" : "delivery.requested", { deliveryId }]);
    }
    return {
      duplicate: false,
      message: {
...
export async function getMessage(id: string, serviceId: string) {
  ...
  }>(`SELECT m.id,p.name AS product,t.key AS template,m.recipient_count,m.accepted_at,m.reference_id FROM messages m JOIN products p ON p.id=m.product_id JOIN templates t ON t.id=m.template_id WHERE m.id=$1 AND m.service_id=$2`, [id, serviceId]);
  ...
  const deliveries = (await query<{
    ...
    updated_at: Date
  }>("SELECT id,recipient_email,lifecycle_status,engagement_status,compliance_status,updated_at FROM deliveries WHERE message_id=$1 ORDER BY queued_at", [id])).rows;
  return {
    ...message,
    status: deliveries.every(d => d.lifecycle_status === "delivered") ? "delivered" : deliveries.some(d => ["queued", "submitting", "accepted", "deferred", "unknown"].includes(d.lifecycle_status)) ? "in_progress" : "completed",
    deliveries
  }
};
